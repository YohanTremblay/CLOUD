...
def sendStatus(status):
    jsonString = {"message": status}
    mqtt_client.publish(f"{PROJET}/{FUSEE}/{PORTAIL}/{OPERATION_STATUT}", json.dumps(jsonString))

def connected(client, userdata, flags, rc):
    print("Serveur:\t%s" % secrets["mqtt_broker"])
    print("\nAbonnement au topic...")
    mqtt_client.subscribe(f"{PROJET}/{FUSEE}/{MODULE}/{OPERATION_STATUT}/#")
    mqtt_client.subscribe(f"{PROJET}/{FUSEE}/{PORTAIL}/{OPERATION_STATUT}/{P01}")
